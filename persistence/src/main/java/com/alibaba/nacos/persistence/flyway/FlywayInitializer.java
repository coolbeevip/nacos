package com.alibaba.nacos.persistence.flyway;
...
public class FlywayInitializer implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    public void initialize(ConfigurableApplicationContext configurableApplicationContext) {
        Optional<String> optional = Optional.ofNullable(configurableApplicationContext.getEnvironment().getProperty("spring.flyway.enabled"));
        if (optional.isPresent() && optional.get().toLowerCase().equals("true")) {
            ...
            Flyway flyway = Flyway.configure().locations(locations).table(table).baselineOnMigrate(true).cleanDisabled(true).baselineVersion(baselineVersion).dataSource(url, user, password).load();
            flyway.migrate();
        } else {
